...
def buffer_search_packets(in_bytes):
    ...
    while True:
        ...
        if len(curr_bytes) < 2: # Too short for a length
            print("Partial header found -- no length")
            return
        read_length, = struct.unpack('>H', curr_bytes[0:2]);
        curr_bytes = curr_bytes[2:] # Consume the length
        print("Candidate length: {}".format(read_length))
        if len(curr_bytes) < read_length:
            print("Partial packet found")
        else:
            print("Checksum: {}".format(curr_bytes[read_length-1]))
